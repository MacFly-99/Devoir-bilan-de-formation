  ...
  useEffect(() => {
    const fetchPieces = async () => {
      try {
        ...
        setPieces(data['hydra:member'] || data); // API Platform renvoie les données dans "hydra:member"
        setLoading(false);
      } catch (err) {
  ...
  return (
    <div className="min-h-screen bg-gray-100 p-8">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 max-w-7xl mx-auto">
        {pieces.map((piece) => (
          <div key={piece.id} className="bg-white rounded-xl shadow-md overflow-hidden hover:shadow-lg transition-shadow">
            <img 
